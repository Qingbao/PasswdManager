 ...
 *
 */
package passwdmanager.hig.no.lds;
 ...
 *
 */
public class DG_2_FILE extends DataGroups {
	 ...
	 *
	 * @param data the face image raw data
	 * @param mimeType the mime type of the image data
	 */
	public DG_2_FILE(byte[] data, String mimeType) {
		signatureData = data;
	 ...
	 */
	public DG_2_FILE(InputStream in) throws IOException {
		BERTLVInputStream tlvIn = new BERTLVInputStream(in);
		int tag = tlvIn.readTag();
		if (tag != FileStructure.EF_DG2_TAG) {
		...
		signatureData = (byte[]) dataObject.getValue();
	}
	...
	public String toString() {
		return "DG2File: type " + imageType + " bytes " + signatureData.length;
	}
	...
	public byte[] getEncoded() {
		...
		try {
			BERTLVObject result = new BERTLVObject(EF_DG2_TAG,
					new BERTLVObject(TYPE_TAG, new byte[]{(byte) imageType}));
			result.addSubObject(new BERTLVObject(DATA_TAG, signatureData));
	...
	public static void main(String[] args) {
		try {
			byte[] testArray = new byte[]{0x75, 0x08, (byte) 0x89, 0x01,
				0x03, 0x5F, 0x43, 0x02, (byte) 0xDE, (byte) 0xAD,};
			DG_2_FILE f = new DG_2_FILE(new ByteArrayInputStream(testArray));
