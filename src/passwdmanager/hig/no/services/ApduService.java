...
import passwdmanager.hig.no.utils.CryptoUtils;
...
public class ApduService extends CardService {
    ...
    /**
     * Opens a session by connecting to the card and selecting the 
     * applet.
     */
    ...
    /**
     * Sends a <code>SELECT FILE</code> command to the CARD. Secure
    ...
    /**
     * Sends a <code>READ BINARY</code> command to the card. Secure
     * messaging will be applied to the command and response apdu.
     * 
     * @param wrapper
     *            the secure messaging wrapper to use
    ...
    /**
     * Sends a <code>GET CHALLENGE</code> command to the card.
     * Possibly use secure messaging (EAP).
     * 
    ...
    /**
     * Sends an <code>EXTERNAL AUTHENTICATE</code> command to the card.
     ...
     *         by the card, decrypted (using <code>kEnc</code>) and
