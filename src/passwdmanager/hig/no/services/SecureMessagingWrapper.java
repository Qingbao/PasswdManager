 ...
 *
 * TODO: make this more general (and related routines) to support BAC other
 * configurations
 *
 *
 */
public class SecureMessagingWrapper implements APDUWrapper {
	 ...
	 * provide the necessary cryptographic primitives ("DESede/CBC/Nopadding"
	 * Cipher, "ISO9797Alg3Mac" Mac).
	 */
	 ...
	 * ICAO-TR-PKI
	 *
	 * @param capdu buffer containing the apdu data. It must be large enough to
	...
	private byte[] readDO87(DataInputStream in)
		...
		byte[] data = CryptoUtils.unpad(paddedData);
		return data;
	}
	...
	private boolean checkMac(byte[] rapdu, byte[] cc1) throws GeneralSecurityException {
		try {
			...
			ssc++;
			dataOut.writeLong(ssc);
			byte[] paddedData = CryptoUtils.pad(rapdu, 0, rapdu.length - 2 - 8 - 2);
